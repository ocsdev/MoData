...
def get_stcks(day_from='2014-03-01'):
...
def get_data(url,code=None,day=None,cash=300000):
    try:
        ...
        data =[[day,df['time'].count(),
               ...
               (df['cash'][(df.type=='B') & (df.cash<cash)].sum()-df['cash'][(df.type=='S') & (df.cash<cash)].sum())/10000,
               (df['cash'][(df.type=='B') & (df.cash>=cash)].sum()-df['cash'][(df.type=='S') & (df.cash>=cash)].sum())/10000,
...
if __name__ == '__main__':
    get_stcks()
#     get_data('http://market.finance.sina.com.cn/downxls.php?date=2014-08-01&symbol=sh600215',code='002274',day='2014-07-31')
